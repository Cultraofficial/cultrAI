...
for directory in directories_to_check:
    if os.path.exists(directory):
        ...
        # Clean up filenames (remove file extensions or folder names if needed)
        cleaned_files = [file.replace("/", "_").lower() for file in files_in_dir]
        combined_files.extend(cleaned_files)
    else:
...
duplicate_models = [model for model in already_downloaded_models if model.replace("/", "_").lower() in combined_files]
...
models_to_download = [model for model in already_downloaded_models if model.replace("/", "_").lower() not in combined_files]
